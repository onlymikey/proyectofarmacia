...
class ProductsCRUD:
    ...
    def create_widgets(self):
        ...
        ttk.Label(products_frame, text="Descripción:").grid(row=3, column=0, padx=10, pady=10)
        self.description_entry = ttk.Entry(products_frame)
        self.description_entry.grid(row=3, column=1, padx=10, pady=10)

        ttk.Label(products_frame, text="Precio:").grid(row=4, column=0, padx=10, pady=10)
        ...
        ttk.Button(products_frame, text="Nuevo", command=self.new_product).grid(row=5, column=0, padx=10, pady=10)
        ...
        ttk.Button(products_frame, text="Actualizar", command=self.update_product).grid(row=5, column=2, padx=10, pady=10)
        ttk.Button(products_frame, text="Eliminar", command=self.delete_product).grid(row=5, column=3, padx=10, pady=10)
    ...
    def save_product(self):
        ...
        result = self.product_controller.create_product(upc, name, 0, description, price)
    ...
    def update_product(self):
        ...
        result = self.product_controller.update_product(upc, name, 0, description, price)
        messagebox.showinfo(result['type'], result['message'])
        if result['status']:
            self.clear_fields()
